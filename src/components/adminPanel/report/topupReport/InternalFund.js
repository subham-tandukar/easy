...
export default function InternalFund() {
    ...
    useEffect(() => {
        TopupList();
    }, [chooseCooperative, chooseMember, internalFormValue.status || internalFormValue.fromDate || internalFormValue.toDate])
    ...
    const TopupList = () => {
        ...
        fetch(`${reactURL}MblPayPanel/MoneyTransfer/MoneyTransferReport`, requestOptions)
            .then((result) => {
        ...
        },
        {
            ...
            selector: (row) => {
                return (
                    <>
                        {row.status}
                        <span className={internalFormValue.status === "PENDING" || row.status === "PENDING" ? " " : "d-none"}
                            onClick={() => { handleReverse(row.tranID) }}
                        >
