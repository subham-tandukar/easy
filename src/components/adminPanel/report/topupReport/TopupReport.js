...
export default function TopupReport() {
    ...
    useEffect(() => {
        TopupList();
    }, [chooseCooperative, chooseMember, reportFormValue.fromDate, reportFormValue.toDate, reportFormValue.status || reportFormValue.fromDate || reportFormValue.toDate])
    ...
    const TopupList = () => {
        ...
        fetch(`${reactURL}MblPayPanel/BalanceTransfer/TopupReport`, requestOptions)
            .then((result) => {
    ...
    const DepositorList = () => {
        ...
        fetch(`${reactURL}MblPayPanel/UserMgnt/DepositorUserList`, requestOptions)
            .then((result) => {
                result.json().then((resp) => {
    ...
    const columns = [
        ...
        },
        {
            ...
            selector: (row) => {
                return (
                    <>
                        {row.tranStatus} <span className={reportFormValue.status === "PROCESSING" || row.tranStatus === "PROCESSING" ? " " : "d-none"}
                            onClick={() => { handleDelete(row.tranID) }}
                        >
                            <img src={image} alt="reverse" style={{ height: "20px", width: "20px" }} />
                        </span>
                    </>
                )
            }
    ...
    const searchHandler = (e) => {
        ...
        if (srchQuery) {
            ...
            let srchResult = originalList.filter((list) => {
                return list["userName"].toLowerCase().includes(srchQuery) || list["operator"].toLowerCase().includes(srchQuery);
            });
    ...
    const handleDeleteTrue = async () => {
        ...
        const response = await fetch(`${reactURL}MblPayPanel/BalanceTransfer/CheckProcessingTxns`, {
            method: "POST",
            headers: {
                "Content-Type": "application/json",
                Signature: apisignature
                        ...
                        <DataTable
                            ...
                            subHeaderComponent={
                                <>
                                    ...
                                    <div className="upper-dataTbl me-2">
                                        <label style={{ fontSize: "14px", textAlign: "left", display: "block" }}>Choose Option</label>
                                        <select
                                            style={{ fontSize: "11px" }}
                                            name="snotifiaction"
                                            value={chooseOption}
                                    ...
                                    {chooseOption === "member" && (
                                        <div className="upper-dataTbl me-2">
                                            <label style={{ fontSize: "14px", textAlign: "left", display: "block" }}>Select Member</label>
                                            <select
                                                style={{ fontSize: "11px" }}
                                                name="snotifiaction"
                                                value={chooseMember}
                                    ...
                                    <div className="upper-dataTbl me-2">
                                        <label style={{ fontSize: "14px", textAlign: "left", display: "block" }}>To Date</label>
                                        <Calendar
                                            className="form-control form-control-sm mb-1"
                                            dateFormat="YYYY/MM/DD"
                                            theme="default"
                                    ...
                                    <div className="upper-dataTbl me-2">
                                        <label style={{ fontSize: "14px", textAlign: "left", display: "block" }}>Select Status</label>
                                        <select
                                            style={{ fontSize: "11px" }}
                                            name="status"
                                            className="form-select form-select-sm"
